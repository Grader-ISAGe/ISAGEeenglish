...
/**
 * Defines the editing form for the ISAGEeenglish question type.
 *
 * @package    qtype
 * @subpackage ISAGEeenglish
 * @copyright   
...
/**
 * Generates the output for ISAGEeenglish questions.
 *
 *@copyright  
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//Question outputs
class qtype_ISAGEeenglish_renderer extends qtype_renderer {
...
*/
        ...
        if (!$placeholder) {
            ...
            $result .= html_writer::tag('label', get_string('answer', 'qtype_ISAGEeenglish',
    ...
    public function specific_feedback(question_attempt $qa) {
        $fraction = $qa->get_fraction();
        if(0.9 <=$fraction && $fraction <= 1){return get_string('verygood','qtype_ISAGEeenglish');  }
            else  if(0.75 <= $fraction && $fraction < 0.9){return get_string('good','qtype_ISAGEeenglish');  }
        else  if(0.6 <= $fraction && $fraction < 0.75){return get_string('satisfactory','qtype_ISAGEeenglish');  }
        else  if(0.5 <= $fraction && $fraction < 0.6){return get_string('average','qtype_ISAGEeenglish');  }
        else  if(0.25 <= $fraction && $fraction < 0.5){return get_string('poor','qtype_ISAGEeenglish');  }
                else {return get_string('failed','qtype_ISAGEeenglish');}
    ...
    public function correct_response(question_attempt $qa) {
        ...
        if($fraction == 1)
        return get_string('modelansweris', 'qtype_ISAGEeenglish',
               $answer['answer']);

        if($fraction != 1)
        return get_string('correctansweris', 'qtype_ISAGEeenglish',
               $answer['answer']);
    }
